...
auto Run::skygrid_gammas_move_2() -> void {
  const Pop_model& raw_pop_model = *pop_model_;
  CHECK(typeid(raw_pop_model) == typeid(Skygrid_pop_model));
  auto skygrid_pop_model = std::static_pointer_cast<const Skygrid_pop_model>(pop_model_);

  // Here, the suggested move in Gill et al 2012 is super-smart but super-complicated.  If
  // we consider the dependence of the posterior P on a single gamma_k in the staircase
  // model, we have the following:
  //
  //
  //                            +------------------ B_k ------------------------+
  //                           /                                                 \   |
  //  P dgamma_k \propto exp[-{int_{x_{k-1}}^{x_k} dt k_bar(t) (k_bar(t) - 1) / 2}
  //                        * exp(-gamma_k)] exp(-gamma_k N_{inner,k}) dgamma_k.
  //
  // After changing coordinates to I_k = exp(-gamma_k) and accounting for the associated Jacobian,
  // we get a Gamma distribution:
  //
  //  P dI_k \propto I_k^{N_{inner,k}-1} exp[-B_k I_k].
  //
  // This move simply makes a proposal based on this Gamma distribution, and uses MH to correct for the bias.
  ...
  auto t_min = k == 0 ? -std::numeric_limits<double>::infinity() : skygrid_pop_model->x(k-1);
  auto t_max = k == M ? +std::numeric_limits<double>::infinity() : skygrid_pop_model->x(k);
  ...
  for (auto c = c_min; c <= c_max; ++c) {
    auto a = std::clamp(coalescent_prior_.cell_lbound(c), t_min, t_max);
    auto b = std::clamp(coalescent_prior_.cell_ubound(c), t_min, t_max);
    B_k += 0.5 * Delta * k_c[c] * (k_c[c] - 1.0) * (b - a);
  }
  ...
  for (const auto& node : index_order_traversal(tree_)) {
    if (not tree_.at(node).is_tip()) {
      ...
      auto node_k = skygrid_pop_model->interval_containing_t(t);
  ...
  if (N_inner_k < 1 || B_k == 0) { return; }  // No information in this interval
  
  // WARNING: C++ calls "beta" the *scale* of the distribution, i.e., the reciprocal of the rate
  auto proposed_I_k_dist = std::gamma_distribution<double>{
    static_cast<double>(N_inner_k),
    1.0 / B_k};
  auto delta = -std::log(proposed_I_k_dist(bitgen_)) - skygrid_pop_model->gamma(k);
...
auto Run::skygrid_gammas_hmc_move_2() -> void {
  // This move uses Hamiltonian Monte Carlo (HMC) to perform effective collective moves of the
  // Skygrid population curve.  The curve is specified by a set of values gamma_k (0<=k<=M),
