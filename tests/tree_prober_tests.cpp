...
TEST(Tree_prober_test, invalid_p_initial) {
  ...
  EXPECT_THROW((Tree_prober{branch_counts_by_cat, pop_model, {0.0, 0.0, 0.0}}), std::invalid_argument);

  // Negative values
  EXPECT_THROW((Tree_prober{branch_counts_by_cat, pop_model, {-0.3, -0.2}}), std::out_of_range);

  // Add up to more than 1 (adding up to less is fine: it just means there's an implicit "other" category)
  EXPECT_THROW((Tree_prober{branch_counts_by_cat, pop_model, {0.6, 0.7}}), std::out_of_range);
}
...
TEST(Tree_prober_test, trivial) {
  ...
  auto tree_prober = Tree_prober{branch_counts_by_cat, pop_model, p_initial};
...
TEST(Tree_prober_test, simple) {
  ...
  auto tree_prober = Tree_prober{branch_counts_by_cat, pop_model};
